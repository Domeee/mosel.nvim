...
M.groups = {
  ...
  EndOfBufferNC = { fg = c.bg2, bg = c.bg2 },                   -- filler lines (~) after the end of the buffer. By default, this is highlighted like |hl-NonText|.
  ...
  SignColumnNC = { fg = c.fg, bg = c.bg2 },                     -- column where |signs| are displayed
  ...
  MsgArea = { fg = c.fg2, bold = true },                        -- Area for messages and cmdline
  ...
  NormalBlub = { fg = c.pink, bg = c.green },                   -- normal text
  ...
  StatusLineNC = { bg = c.bg2, fg = c.fg },                     -- status lines of not-current windows Note: if this is equal to "StatusLine" Vim will use "^^^" in the status line of the current window.
